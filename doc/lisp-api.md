...
| `(regexp-opt STRINGS)` | A regexp matching exactly the strings in `STRINGS` **when used whole**. kg's is the simple form — sorted LONGEST FIRST, each member `regexp-quote`d, joined with a bare `\|` — where Emacs returns an optimized trie. No members gives an unmatchable regexp; one member comes back bare. **The result is NOT composable**: there is no group around the alternation (the engine has shy groups as of the R2 pin, but this name does not emit one yet), so text concatenated before or after it binds to only the first or last alternative — `(concat (regexp-opt '("a" "b")) "c")` still matches a bare `"a"` where Emacs' grouped result does not. Behaviour matched to Emacs is direct matching only, until this name adopts the wrapper. Emacs' `PAREN` argument is `wrong-number-of-arguments` |
