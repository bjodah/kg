...
The supported embedding interface is `FE_API_VERSION 14`; `src/lisp_core.c`
...
`FE_LANGUAGE_VERSION 16`, which `src/lisp_core.c` also asserts at compile
...
has used since Phase 8) and puts a `stringp` arm in front of each. Phase 25,
which gives fe length-bearing strings, is where those wrappers retire; fe
records the byte behaviour as its own divergence, `sequence-length-string-bytes`.
