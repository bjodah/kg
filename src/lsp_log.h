 ...
 * pushes it past the cap evicts whole lines from the head until it fits
 * again -- oldest first, never a partial line, and the buffer is rewritten
 * from the store when that happens so what is on screen is what is kept.
 *
 ...
 * rewritten in one append on the rare occasion the cap is crossed.  A log
 * that grew without a bound would be a memory leak with a friendly name,
 * since a misconfigured server can write to its stderr forever. */
...
/* Append `len` bytes (the caller's own newline included) and evict from
 * the head until the store holds at most `max_bytes`.  `*trimmed` says
 * whether anything was evicted, which is what tells an append apart from a
 * rewrite.  False, with the store unchanged, when memory ran out. */
