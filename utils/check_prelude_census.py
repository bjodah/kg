...
def write_manifest(path: Path, measured: dict) -> None:
	data = {
		...
		"note": ("What the embedded Lisp prelude (lisp/prelude.el) costs at "
			 ...
			 "plus Fe's payload region, whose four USE numbers left "
			 "zero at FE_LANGUAGE_VERSION 17, where a string's bytes "
			 "became payload and a symbol's name is a string: the "
			 "prelude interns hundreds of names, so it publishes "
			 "blocks and the collection it already ran compacts "
			 "them. No "
