...
;;;
;;; Four measured constraints shaped it, and are worth knowing before
;;; editing it:
;;;
;;;   * a `throw' could not cross a native re-entry boundary in kg (the
;;;     then-recorded catch-throw-reachability divergence), so every
;;;     catch and its throws live inside this file, with no
;;;     `save-excursion' or `with-current-buffer' in between.
;;;     `pipeline-text.el' never throws across one either.  THAT
;;;     DIVERGENCE IS CLOSED as of Phase 11 for exactly those two forms,
;;;     which became prelude `unwind-protect' macros with no native
;;;     frame between the throw and the catch.  Every OTHER native
;;;     re-entry -- a hook, a process filter, a nested `command-execute'
;;;     -- is still a wall, so the shape this file avoids is still worth
;;;     avoiding; the reason is narrower than when it was written.
...
;;;     added.  kg reads the cookie as the comment it is, so the line
;;;     costs kg nothing and is what makes "runs unchanged under Emacs
;;;     31" true rather than nearly true.  (It said "kg has only lexical
;;;     binding" until Phase 11 gave kg dynamic binding too.  The line
;;;     still costs kg nothing: kg's dynamic binding follows the marks
;;;     `defvar'/`defconst' set on a symbol, never a per-file cookie, so
;;;     nothing in this file binds dynamically either way.)
