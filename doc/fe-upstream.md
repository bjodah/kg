...
Phase 24 moved **both**, in the one fe commit that lands vectors:
`FE_LANGUAGE_VERSION` 15 → **16** and `FE_API_VERSION` 13 → **14**, with
`FeVersion` `"19.0"` → **`"20.0"`**. Both `static_assert`s in
`src/lisp_core.c` fired at this pin, which is the two-macro scheme working.
The language bump is a break in the ordinary direction — `[1 2 3]` used to
be a named read error and is now a vector — and the API bump is a real ABI
break rather than an addition: `FeTVector` is INSERTED after `FeTString` in
the `FeType` enum, renumbering `FeTPtr` and the three `FeTFex*` slots, which
is the placement decision sub-plan 05A made for `FeTInteger` and for the
same reason. Four new C entry points come with it — `FeMakeVector`,
`FeVectorLength`, `FeVectorRef`, `FeVectorSet` — and deliberately no
borrowed-elements accessor, because the storage such a borrow would name is
the storage `CompactPayloads` moves.

**kg's side of this pin is one decision with numbers attached: it stops
refusing the carve.** A vector IS a header plus a payload block, so a
context opened with `FePayloadPercentNone` cannot build a vector of any
length, zero included, and answers `(payload-exhaustion)`; `lisp_arena_options`
therefore passes `FeDefaultPayloadPercent`. The whole partition moves once,
here, and every figure is measured at this pin rather than carried:
`FeMinimumArenaSize()` 66544 → **67664 bytes** (+1120, which fe's commit
accounts for exactly: 57 slots for the eight new primitives and 13 for the
`end-of-file` condition row, all interned at context open). At kg's compiled
10 MiB default the arena partitions to **440489 object slots / 10916 frames
/ 2344064 payload bytes**, against 586993 / 10916 / 0 for the same arena
uncarved and 586986 / 10917 at the previous pin. At 1 MiB it is **42358 /
1085 / 220704** (56152 uncarved, 56145 at the previous pin), and at 2 MiB —
the size `test/pty/lisp-arena-bytes-knob.yaml` opens — **86595 / 2178 /
456632** (115134 uncarved, 115127 before).

The **Phase-B floor re-derives from the census under the carve**, which is
what it is for: `lisp_arena_min_size` 640 KiB → **768 KiB**. The prelude's
`reachable_live_objects` measures **10241** (10016 before: +169, fe's +70
interned objects plus the prelude's four new definitions), so the floor has
to open at least 3 × 10241 = **30723** cells. Carved, 640 KiB opens 25769 and
704 KiB opens 28534 and 736 KiB opens 29917 — all short — and **768 KiB
opens 31299**, the first 64 KiB step that clears it. `test/test_lisp.c`'s
`test_arena_floor_matches_census()` re-derives both halves from the refusal
message and the census file rather than from constants, so it needed no edit
and is the check that this arithmetic stays true.

The rest of the kg side is the number churn that one decision drags, all of
it corrected in the pin-moving commit: `.ci/prelude-startup-census.json`
(`peak_live_objects` 10993 → **11248**, `reachable_live_objects` 10016 →
**10241**, `embedded_bytes` 71720 → **74840**, `definition_count` 122 →
**126**, and `payload_capacity_bytes` off its ceiling of zero to the measured
**2344064** while the four USE fields stay at zero — the prelude defines
vector-aware names without building a vector); the five PTY cases that name
a slot count; `test/test_cmd.c` and `test/test_perf.c`, whose "the payload
region is five zeros" assertions become "capacity nonzero, use zero"; and
`src/lisp_core.c`'s three arena comments.

**kg keeps its own `length` and `elt` for strings, and only for strings.**
fe generalised both at its own layer, which is right — a `length` that knew
two of three sequence types would be exactly the deliberately broken
interval the master plan's Phase 24 exists to avoid — but a fe string is a
chain of BYTES and fe counts and indexes them, where Emacs counts and
indexes CHARACTERS. `lisp/prelude.el` therefore captures the two primitives
as `internal--fe-length` and `internal--fe-elt` (the idiom `internal--let`
has used since Phase 8) and puts a `stringp` arm in front of each. This
paragraph used to end "Phase 25, which gives fe length-bearing strings, is
where those wrappers retire", and that prediction was wrong: Phase 25 gave
a string a LENGTH, not a character count, and froze `length`/`aref`/`elt`
as BYTE operations in fe's own language contract. So the wrappers stay,
and Phase 25 added a third — `aref`, the one name the freeze found
unwrapped — rather than retiring the first two. fe records the byte
behaviour as its own divergence, `sequence-length-string-bytes`.

Phase 25 moved **both** again, in the one fe commit that lands strings:
`FE_LANGUAGE_VERSION` 16 → **17** and `FE_API_VERSION` 14 → **15**, with
`FeVersion` `"20.0"` → **`"21.0"`**. A string was a cdr chain of cells
carrying seven bytes each, terminated by a NUL inside the last one; it is
one object now, its byte LENGTH in the bytes of its `car` word the type
tag does not use and its bytes in the payload region. A symbol's name is
such a string, with symbol identity unchanged. The language break is that
`"a\0b"` reads as a three-byte string where the escape was a named read
error, and that `(aset STRING INDEX BYTE)` writes a byte in place where
every `aset` on a string was refused. The API break is that
`FePayloadPercentNone` is **removed**: a context with no region cannot
finish opening, so "no region" stopped being a partition a host can ask
for. `FeMakeStringBytes` and `FeStringBytes` are the additions;
`FeStringByteLength`/`FeCopyStringBytes` are untouched and were always
binary-safe, which is why kg's C boundary needed no lifetime migration at
all (`fe/doc/payload-pointer-census.md`, section D).

**kg's side of this pin is a number churn and a semantic question, and no
lifetime work.** The partition moves once more, measured at this pin:
`FeMinimumArenaSize()` 67664 → **72136 bytes**, which now funds the 6232
bytes the core names' own blocks occupy; the 10 MiB default partitions to
**440190 object slots / 10911 frames / 2349296 payload bytes** (440489 /
10916 / 2344064 before), 1 MiB to **42059 / 1081 / 225928** and 2 MiB to
**86296 / 2173 / 461856**. There is no longer an "uncarved" column to
compare against. `lisp_arena_min_size` stays at **768 KiB** and is
re-derived rather than moved: it opens 31000 slots against the 26847 that
are three times the prelude's `reachable_live_objects` of 8949, which is
inside the `[3×, 6×)` window `test/test_lisp.c`'s
`test_arena_floor_matches_census()` enforces. The census banks four falls
and three rises: `peak_live_objects` 11248 → **9956** and
`reachable_live_objects` 10241 → **8949** (a string is one object now
instead of one cell per seven bytes), `payload_capacity_bytes` 2344064 →
**2349296**, and `payload_live_bytes` / `payload_peak_bytes` /
`payload_compactions` off their ceilings of zero to **35496 / 66128 / 1** —
the prelude's interned names, which are exactly the objects that moved
from cells into the region.

**The kg-side semantic question the pin raises is NUL truncation, not
lifetimes.** kg's twenty `copy_fe_string()`-shaped sites copy through an
API that never handed out an interior pointer, so nothing about their
lifetime changed; what changed is that the bytes they copy may now contain
a NUL, and a site that then treats the copy as a C string truncates
silently. Every such site is reviewed one by one, with a verdict recorded
for each, in `doc/lisp-string-nul-policy.md`.
...
**kg passed `FePayloadPercentNone` explicitly at this pin**
(`src/lisp_core.c`'s `lisp_arena_options`), not the record's default of 25%:
nothing kg ran could own a payload yet, so a carve would have cost a quarter
of the cells (42335 instead of 56145 at 1 MiB) for storage nothing could
allocate from. The arena partition was therefore **unmoved at this pin** —
56145 slots / 1086 frames at 1 MiB, `FeMinimumArenaSize()` still 66544
bytes, the prelude census still 10993 / 10016 — and no PTY case, oracle
snapshot or arena figure changed.  (Every arena figure in this table is
quoted at 1 MiB, and stays quoted there so the pins remain comparable, but
1 MiB stopped being kg's compiled default after this pin: Phase B of
`doc/plans/2026-08-19-fe-simplification-and-cheap-compat.md` made it
10 MiB.)  **Phase 24 is where that refusal ended**, and the row below is
where its numbers are: a vector's elements live in the payload region, so a
context that carves none cannot build one. What kg gained at THIS pin is the
reporting: the five fields are appended, last, to `M-x lisp-arena-stats`,
`test/kgbatch -g` and `test/prelude_gc_probe`, and are mirrored into
`KG_PERF_LISP_PAYLOAD_*`. `.ci/prelude-startup-census.json` and
`test/test_perf.c` held all five at **zero** here; since Phase 24 they hold
the four USE fields there and let capacity be the carve, which is the same
early-use ratchet one notch up.
...
| **Vectors, and the sequence contract they drag with them** (fe `9218d1a`, `1fd0dce`; `FE_LANGUAGE_VERSION` 15 → **16**, `FE_API_VERSION` 13 → **14**, `FeVersion` "20.0") | Phase 24 of kg's Elisp data-model program, and the payload region's first real tenant. A vector IS an `FeObject` header plus a payload block whose traced children are its elements — no length word, no capacity, no element array of its own — so `length` is the block's `children` count and `aref`/`aset` are a field read and one multiply-add. The surface: `[...]` reader syntax with re-readable printing; `vector`, `make-vector`, `vectorp`, `aref`, `aset`, `vconcat`; `length` and `elt` generic over lists, strings and vectors at **fe's** layer rather than a host's prelude; and four public C entry points. Every answer was frozen against Emacs 31.0.91 BEFORE the code existed (kg's `test/lisp-compat/cases/vector-*.json`, Phase 24.0), and the details measurement bought are the ones a native implementation gets wrong silently: `args-out-of-range` data is `(SEQUENCE INDEX)` with the sequence FIRST; `(aref 5 0)` names `arrayp`, not `vectorp`; `(elt '(1 2) 9)` is nil while `(elt [1 2] 9)` raises; `(vconcat "ab")` is `[97 98]`; `make-vector` STORES its init, so every slot is `eq` to every other; and `[1 2` raises **`end-of-file`**, Emacs' generic incomplete-input condition, which now also names an unclosed LIST (message text unchanged, so kg's own assertion on it still holds). **SELF-REFERENTIAL PRINTING is fe's kept divergence**, decided rather than discovered: Emacs prints `"[0 #0]"` at `print-circle`'s nil default and `"#1=[0 #1#]"` with it bound to t; fe keeps its bounded writer, because `#N` numbering costs 256 `FeObject*` in every `Writer` including the one `RenderErrorMessage` runs where there may be no arena left, the spelling it buys is lossy and still does not read back, and fe already prints the analogous self-referential LIST as bounded nesting. `1fd0dce` is a cppcheck annotation on top of it and changes nothing. | **The whole partition moves, once, and kg stops refusing the carve**: a context opened with `FePayloadPercentNone` cannot build a vector of any length, so `src/lisp_core.c`'s `lisp_arena_options` passes `FeDefaultPayloadPercent`. Measured here, never carried: `FeMinimumArenaSize()` 66544 → **67664**; the 10 MiB default **440489 slots / 10916 frames / 2344064 payload bytes** (586993 / 10916 / 0 uncarved); 1 MiB **42358 / 1085 / 220704** (56152 uncarved); 2 MiB **86595 / 2178 / 456632**. The Phase-B floor re-derives from the census UNDER the carve — `reachable_live_objects` 10016 → **10241**, so 3× is 30723, and 640/704/736 KiB open 25769/28534/29917 cells while **768 KiB opens 31299** — `lisp_arena_min_size` 640 KiB → **768 KiB**. kg's prelude keeps its own `length` and `elt` for STRINGS only (fe counts bytes, Emacs counts characters), capturing fe's as `internal--fe-length`/`internal--fe-elt`, and generalises `mapcar`, `mapc`, `mapconcat`, `append`, `copy-sequence` and the `seq-` shims through one new helper, `internal--seq-to-list`. That closed a divergence 24.0 found and did not aim at: `(append "ab" nil)` is `(97 98)` here now, as in Emacs. **34 oracle cases flipped** from recorded divergence to agreement, including both `sel-frontier` cases — the synthetic package-shaped file now loads clean — and `phase8-reader-vector`; the two self-referential printing cases stay divergent with fe's reason cited. Census: `peak_live_objects` 10993 → **11248**, `embedded_bytes` 71720 → **74840**, `definition_count` 122 → **126**, `payload_capacity_bytes` off zero to **2344064** with the four USE fields still held at zero. |
| **Strings with a length, over the payload region** (fe `48338f4`..`8718046`; `FE_LANGUAGE_VERSION` 16 → **17**, `FE_API_VERSION` 14 → **15**, `FeVersion` "21.0") | Phase 25 of kg's Elisp data-model program, and the payload region's universal tenant: there is no longer such a thing as an fe context whose region is empty. A string was a cdr chain of cells carrying seven bytes each and terminated by a NUL inside the last one; it is ONE object now, its byte LENGTH in the bytes of its `car` word that the type tag does not use, its bytes a payload block that the compactor moves and the header does not. A symbol's name is such a string, and symbol identity is unchanged: interning still walks `symbol_list` and still compares by name, through an accessor rather than a chain — and rejects most candidates on their LENGTH before touching a byte. `Equal`'s string arm was rewritten rather than re-derived, exactly as the interior-pointer census predicted, because it compared whole `car` WORDS and no grep for the accessor would have found it. **What the language gains** is that a NUL is DATA: `"a\0b"` reads as three bytes where the escape was the named read error `unsupported read syntax: NUL character in string`, and `(aset STRING INDEX BYTE)` writes a byte in place and answers it where every `aset` on a string was refused. `length`, `aref` and `elt` still answer BYTES — that is fe's frozen contract, not an omission, and it is why kg's prelude wrappers stayed. **Two kept divergences from Emacs, both in the WRITER and both recorded**: an embedded NUL prints as `\000` (three digits, so a digit after it stays a digit) where Emacs emits the raw byte, because `FeReadFn` spells end of input as 0 and a raw NUL is where fe's own reader would stop — escaping it is what makes print-read-`equal` hold, this phase's first required gate; and a byte that is not a character prints raw where Emacs escapes it octally. **`FeWriteFn`'s allocation contract is decided here**, in favour of allowing it: the printer re-derives its payload addresses after every callback, which is a promise fe keeps rather than one every host has to. | **The partition moves once more and kg's C boundary does not move at all.** `FePayloadPercentNone` is removed, so `lisp_arena_options` keeps asking for `FeDefaultPayloadPercent` and that percentage now divides only the SURPLUS above the floor `FeMinimumArenaSize()` funds. Measured here, never carried: `FeMinimumArenaSize()` 67664 → **72136** (+4472, the 6232 bytes of core-name blocks less what those names no longer spend on cells); the 10 MiB default **440190 slots / 10911 frames / 2349296 payload bytes** (440489 / 10916 / 2344064 before), 1 MiB **42059 / 1081 / 225928**, 2 MiB **86296 / 2173 / 461856**, and no uncarved column to compare against any more. `lisp_arena_min_size` is RE-DERIVED and stays at **768 KiB**: it opens 31000 slots against 3 × 8949 = 26847, inside the `[3×, 6×)` window `test_arena_floor_matches_census()` enforces. Census: `peak_live_objects` 11248 → **9956**, `reachable_live_objects` 10241 → **8949** — a string is one object instead of one cell per seven bytes — and the payload USE fields off their ceilings of zero to `payload_live_bytes` **35496**, `payload_peak_bytes` **66128**, `payload_compactions` **1**, which are the prelude's own interned names. kg's twenty `copy_fe_string()`-shaped sites needed NO lifetime migration (`fe.h` never handed out an interior pointer and the two-call API was always binary-safe); what they needed was a NUL-truncation verdict each, which `doc/lisp-string-nul-policy.md` records. |
...
| `*`, `+`, `?` and `\{` with nothing to repeat are the literal characters | Emacs reads them that way at the start of a pattern, group or alternative | inside a group the `?` is ordinary too (`\(?:*\)` matches `"*"`); at `\(` itself it is the reserved opening below, never a literal |
| Shy groups, `\(?:...\)`, and `\(?` reserved for them (tiny-regex-c `29fc9fb`) | Emacs' own `regexp-opt` emits the wrapper, and a constructed regexp is only composable when it is one atom — a capturing group would renumber the caller's captures instead. Upstream has neither the spelling nor the reservation, and the escaped-character fall-through made `\(?:a\)` match `"?:a"`: a pattern quietly matching the wrong thing. | Compile-time only — the opener answers which capture register the group fills and 0 means none, which `group_span()` already read as "no slot", so a shy group is a group everywhere in the matcher and a capture nowhere. Numbering needs no second pass: `\(?:a\)\(b\)` has `b` as capture 1. Every other `\(?` spelling is now `RE_STATUS_BAD_PATTERN`. |
| A bounded match window, `re_exec_bounded()` with `RE_LIMIT_NONE` (tiny-regex-c `1bcd57b`) | Emacs' search BOUND bounds CONSUMPTION, not just the reported span, and kg had no way to say so: `lisp_search_row_forward()` filtered the engine's unbounded answer, so a preferred branch crossing BOUND killed the whole row — adversarial-review Finding 2. | The limit is one comparison in `match_atom()`'s glyph loop, which is the single place this engine consumes, so backtracking sees it: a greedy branch hands characters back, an alternative that cannot fit loses to a later one at the same start, and the scan goes on to later starts. NOT a truncation — `\'`/`$` keep testing `text`'s own terminator and `` \` ``/`^` `text` itself. No existing signature moved; `re_exec()` and `re_exec_with_options()` answer exactly as before. |
| An explicitly numbered group, `\(?1:...\)`, is a bad pattern | Emacs accepts it; placing a group at a capture number the pattern NAMES rather than at the one its position gives it is not something this compiler can express, and refusing beats misreading | joins the tenth-capture-group refusal in `re.h`'s "what is a bad pattern" section |
| At most `RE_MAX_SPANS` groups may be open at once, shy ones included | the compiler's parse stack is that tall, and shy groups are the first spelling that can reach the height without also asking for a register | Emacs has neither this nor the capture ceiling above it; both are `RE_STATUS_BAD_PATTERN` |
...
| The matching budget is per execution, with `re_exec_with_options()` for per-call limits and cancellation | it used to live in file-scope statics, so a nested or concurrent match spent the running one's allowance | kg calls `re_exec()` unbounded and `re_exec_bounded()` where it has a BOUND; the options are available for a future C-g-aware search |
...
| Emacs' subject anchors `` \` `` and `\'`, compiled to the same BEGIN and END nodes `^` and `$` produce (tiny-regex-c `8e62ba7`) | Upstream has neither spelling, and the escaped-character fall-through turned each into the literal punctuation it is spelled with — so `` \`abc `` MATCHED "x`abc" instead of failing, which is a pattern quietly matching the wrong thing rather than one that errors. kg presents regexps as Emacs', and `s.el`'s `s-trim` is written with both. | Nothing else moves: they are an ALIAS, because BEGIN already holds only at the subject's start (never a line's, never `re_exec()`'s `start_offset`) and END only at its end. Everything downstream follows — the leading-anchor scan cut-off, the nothing-to-repeat rule (`` \`* `` matches `"*"`, as `^*` does and as Emacs does), and `re_string()`, which renders both as `^`/`$`. They inherit the anchors' one divergence too: Emacs reads `$*` and `\'*` as a QUANTIFIED assertion matching the empty string, kg as a literal `*`. |
...
- **Hash tables, keyword arguments, a byte compiler.** All need new
  object types beyond what the current core provides. This bullet used to
  open with "Dynamic binding"; Phase 11's special-variable row above is
  where that stopped being true, and the reason it was wrong is
  instructive — shallow binding needs no new object type at all, only two
  bits on a symbol and a frame that remembers what it overwrote. It then
  carried **vectors** for eight more phases, on the grounds that a vector
  needs an object type the core does not have. That was correct and is
  now spent: Phase 23 built the payload region and Phase 24 made a vector
  its first tenant, so the type exists and `[1 2 3]` reads. The Phase 8
  reader row's `[1 2 3]` named read error is history, not policy;
  `#:sym`, `#s(…)` and a bare `#` are the half of that row still
  standing.
