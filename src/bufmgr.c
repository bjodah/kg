...
static struct editor_syntax ibuffer_syntax
    = { KG_MODE_IBUFFER, "IBuffer", NULL, "", NULL };
    ...
    int *matches, int *total, int *flen)
{
	...
	*matches = *total > PICKER_MAX_ENTRIES ? PICKER_MAX_ENTRIES
	...
	editor.echo_cursor_col = minibuf_prompt_cursor_col(
	    prompt, (int)strlen(prompt), buf, cursor);
    ...
    const struct path_entry *entries, int literal, int overflow)
{
	...
	/* Every overflow exit says so in the echo area, and
	 * minibuf_prompt_done() keeps that message (it clears only on
	 * cancellation).  The interactive callers then act on the non-ACCEPTED
	 * result by doing nothing, so C-x C-f dismisses with a visible reason
	 * rather than either opening a truncated path -- which is what losing
	 * the counter used to do -- or raising at the user. */
    ...
    int fd, const char *prompt, char *buf, int bufsize)
{
	 ...
	 * balanced the same way by minibuf_prompt_done(), this function's one
	 * exit path too. */
	...
	while (1) {
		...
		if (KEY_IN_LIST(erase_keys, c)) {
			...
			return minibuf_prompt_done(MINIBUF_CANCELLED);
			...
			editor_picker_popup_clear();
			return minibuf_prompt_done(
			    action == PATH_ACCEPT_OVERFLOW ? MINIBUF_OVERFLOW
							   : MINIBUF_ACCEPTED);
    ...
    int outsize, enum buf_name_mode mode, int *picked)
{
	...
	*picked = -1;
	 ...
	 * minibuf_prompt_done(); the refusal above returns before the pair
	 * opens. */
	kg_event_prompt_enter();
