 ...
 *
 * framed_io_new() takes ownership of every nonnegative descriptor passed
 ...
 * delivery.  scan_next() advances the hold without consuming bytes;
 ...
 * or the channel is exhausted.  scan_next() returns true only with a
 * complete held line or bounded piece.  A scanned pointer is valid only
 * until the next scan, fill, next, release_hold, or close call. */
struct framed_line;
