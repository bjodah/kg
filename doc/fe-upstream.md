...
| Symbol storage has a dormant function cell (sub-plan 04B) | Phase 4 needs independent value and function namespaces, but this behavior-neutral representation slice must leave all existing lookup rules alone | `CDR(sym) = ((name . function) . value)` is private behind `SymbolName`, `SymbolBindingCell`, `SymbolFunction`, and `SetSymbolFunction`; the cell adds one arena object per core symbol, so `FeMinimumArenaSize()` is now **54656 bytes** and kg's 1 MiB arena has `frame_capacity` **1099** (not the earlier 53840/1100 values in the `GcStackSize` row above) |
