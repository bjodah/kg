...
- ~~**Lisp bindings for the xref commands.**~~  Done by Phase 17's
  ...
  as from a key.  The 2026-08-11 merge extended the same verdict to the
  four commands the rest of the protocol added: `lsp-diagnostics`,
  `lsp-diagnostics-goto`, `lsp-hover` and `completion-at-point` read no
  terminal, and `lsp-rename` reads one (`CMD_READS_TERMINAL`), which is
  what the audit says a prompt costs rather than what it forbids.
