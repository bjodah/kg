...
| `(regexp-opt STRINGS)` | A regexp matching exactly the strings in `STRINGS`. kg's is the simple form — sorted LONGEST FIRST, each member `regexp-quote`d, joined with a bare `\|` — where Emacs returns an optimized trie; what is guaranteed is the behaviour, not the spelling. No members gives an unmatchable regexp; one member comes back bare. There is no group around the alternation (kg's engine has no shy group), and Emacs' `PAREN` argument is `wrong-number-of-arguments` |
